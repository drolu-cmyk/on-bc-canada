...
Agents plan and generate changes from supplied repository context. Deterministic
...
class ImplementationDeliveryGraph:
    ...
    def definition() -> GraphDefinition:
        ...
        def agent(actor_id: str, authority: str = "A1") -> ActorRef:
            return ActorRef(actor_id=actor_id, kind="agent", authority=authority)
        ...
        return GraphDefinition(
            ...
            nodes=(
                ...
                GraphNode("change_plan", agent("implementation-planning-agent"), "implementation.plan", "implementation.plan"),
    ...
    def register(self) -> None:
        ...
        validators = {
            ...
            "implementation.review": lambda result: bool(result.patch.get("latest_review", {}).get("status")),
    ...
    def _plan(self, state: dict[str, Any]) -> NodeResult:
        ...
        if not required.issubset(planned):
            raise ValueError("implementation plan omitted an operator-required verification")
        return NodeResult(
            patch={"implementation_plan": plan},
            evidence=[{"type": "implementation_plan", "verification_count": len(plan.get("verification_ids", []))}],
        )
    ...
    def _delivery_assurance(state: dict[str, Any]) -> NodeResult:
        ...
        failed_verifications = [
            item["verification_id"] for item in state["verification_results"] if not item["passed"]
        ]
