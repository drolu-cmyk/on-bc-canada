...
def build_implementation_agents(model: str | None = None) -> ImplementationAgentSet:
    ...
    generator = Agent(
        ...
        instructions=shared + (
            ...
            "Never use partial patches or ellipses. Do not touch files outside the implementation plan. Keep the change set minimal and internally consistent."
        ),
...
class OpenAIImplementationProvider:
    ...
    def plan_changes(self, request: dict[str, Any], repository_context: list[dict[str, Any]]) -> dict[str, Any]:
        return self._run(
            self.agents.planner,
            "Plan the reversible staging implementation.",
            {
                "request": request,
    ...
    def generate_changes(self, request: dict[str, Any], repository_context: list[dict[str, Any]], plan: dict[str, Any]) -> dict[str, Any]:
        return self._run(
            ...
            {"request": request, "repository_context": repository_context, "implementation_plan": plan},
            ChangeSetOutput,
        )
    ...
    def review_code(self, request: dict[str, Any], plan: dict[str, Any], applied_changes: list[dict[str, Any]], verification: list[dict[str, Any]]) -> dict[str, Any]:
        return self._run(
            ...
            {"request": request, "plan": plan, "applied_changes": applied_changes, "verification": verification},
