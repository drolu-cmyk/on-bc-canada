...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--learning-db", default=str(DEFAULT_LEARNING_DB), help="Learning Graph SQLite path.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "assign":
            result = progress.assign_active_path(
                learning_store=LearningGraphStore(args.learning_db),
            ...
            result = progress.complete_practice_unit(
                args.instance_id,
                args.unit_id,
    ...
    except (KeyError, ValueError, sqlite3.IntegrityError if False else ValueError) as exc:
