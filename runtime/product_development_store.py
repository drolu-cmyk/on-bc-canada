"""Durable SQLite state for Product Development Graph executions."""
...
class ProductDevelopmentStore:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS product_executions (
                ...
                CREATE TABLE IF NOT EXISTS product_release_records (
                    execution_id TEXT PRIMARY KEY,
                    release_record_json TEXT NOT NULL,
                    recorded_at TEXT NOT NULL,
                    FOREIGN KEY(execution_id) REFERENCES product_executions(execution_id)
                );
                """
            )
    ...
    def save_execution(self, execution: GraphExecution, ledger: EventLedger) -> None:
        with self._connect() as connection:
            ...
            record = execution.state.get("release_record")
            if execution.status == "completed" and record is not None:
                connection.execute(
                    """
                    INSERT INTO product_release_records (execution_id, release_record_json, recorded_at)
                    VALUES (?, ?, ?)
                    ON CONFLICT(execution_id) DO UPDATE SET
                        release_record_json=excluded.release_record_json,
                        recorded_at=excluded.recorded_at
                    """,
                    (execution.execution_id, _dumps(record), _utc_now()),
                )

    def load_execution(self, execution_id: str) -> tuple[GraphExecution, EventLedger]:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM product_executions WHERE execution_id = ?",
                (execution_id,),
            ).fetchone()
        if row is None:
            raise KeyError(f"product execution not found: {execution_id}")
    ...
    def get_release_record(self, execution_id: str) -> dict[str, Any] | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT release_record_json FROM product_release_records WHERE execution_id = ?",
                (execution_id,),
            ).fetchone()
        return json.loads(row["release_record_json"]) if row else None
